...
import { fetchVeteranById, updateVeteran } from '../../api/api';

// Тип для формы редактирования ветерана
...
export const EditVeteranPage: React.FC = () => {
    ...
    // Загрузка данных ветерана для редактирования
    useEffect(() => {
        const fetchVeteranData = async () => {
            ...
            try {
                setIsLoading(true);
                const veteran = await fetchVeteranById(id);
                
                // Преобразуем формат данных для формы
                setFormData({
                    firstName: veteran.firstName || '',
                    lastName: veteran.lastName || '',
                    middleName: veteran.middleName || '',
                    birthDate: veteran.birthDate ? new Date(veteran.birthDate).toISOString().split('T')[0] : '',
                    deathDate: veteran.deathDate ? new Date(veteran.deathDate).toISOString().split('T')[0] : '',
                    biography: veteran.biography || '',
                    rank: veteran.rank || '',
                    awards: Array.isArray(veteran.awards) ? veteran.awards.join(', ') : veteran.awards || '',
                    militaryUnit: veteran.militaryUnit || '',
                    battles: Array.isArray(veteran.battles) ? veteran.battles.join(', ') : veteran.battles || '',
                    imageUrl: veteran.imageUrl || ''
                });
            } catch (error) {
                console.error('Ошибка при загрузке данных ветерана:', error);
                setStatusMessage({
                    type: 'error',
                    text: 'Ошибка при загрузке данных ветерана'
                });
            } finally {
                setIsLoading(false);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatusMessage({
                type: 'success',
                text: 'Данные ветерана успешно обновлены'
            });
            ...
            console.error('Ошибка при обновлении данных ветерана:', error);
            setStatusMessage({
                type: 'error',
                text: error instanceof Error ? error.message : 'Ошибка при обновлении данных ветерана'
            });
        } finally {
        ...
        <AdminLayout>
            <div className="pb-8">
                <div className="flex items-center justify-between mb-6">
                    <h1 className="text-2xl font-semibold text-gray-800">Редактирование ветерана</h1>
                    <button
                        type="button"
